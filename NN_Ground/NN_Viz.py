...
class Window(QMainWindow):
    def __init__(self):
        ...
        if self.draw_lattice == False:
            self.bonds = NNG.bond_list(self.seed, self.N, self.PBC, self.xwidth, self.yheight)
            self.Jij = NNG.make_Jij(self.N, self.bonds, self.lattice)
        self.cnfg = 0
        self.configuration = list(map(int,list(bin(self.cnfg)[2:].zfill(self.N))))
        self.center = (500,375)
        self.coordList = NNG.spinCoords(self.center, self.xwidth, self.yheight, self.spacing)
        self.plaq = NNG.make_plaquettes(self.PBC, self.lattice, self.N, self.xwidth, self.yheight)
        self.f_plaq = NNG.frustrated(self.Jij, self.plaq)

        self.node_pairs = NNG.plaq_pairing(self.f_plaq, self.coordList, self.PBC, self.xwidth, self.yheight, self.spacing)
        init_ground = NNG.initial_ground(self.node_pairs, self.xwidth, self.yheight)
        self.p_pairings = init_ground[0]
        self.ground_distance = init_ground[1]

        self.v_edges = NNG.viable_edges(self.node_pairs, self.p_pairings, self.ground_distance, self.f_plaq, self.xwidth, self.yheight)
        self.matchings = NNG.plaq_groups(self.v_edges, self.f_plaq, self.ground_distance)
        self.string_groups = NNG.add_all_strings(self.matchings, self.lattice, self.coordList)
        self.string_index = 0
        self.b_bonds = NNG.broken_bonds(self.string_groups, self.N, self.coordList, self.xwidth, self.yheight)
        true_ground = NNG.make_config(self.b_bonds, self.Jij, self.N, self.xwidth, self.lattice, self.string_groups)
        ...
        self.scountLabel.setText("Unsatisfied: " + str(self.ground_distance))
